...
from nndl.mamba.selective_scan_interface import selective_scan_fn, mamba_inner_fn, bimamba_inner_fn, mamba_inner_fn_no_out_proj
...
class Mamba(nn.Module):
    ...
    def forward(self, a_hidden_states, v_hidden_states, a_inference_params=None, v_inference_params=None):
        ...
        """
        # print(f"a_hidden_states 的形状: {a_hidden_states.shape}")
        # print(f"v_hidden_states 的形状: {v_hidden_states.shape}")
        # assert a_hidden_states.shape == v_hidden_states.shape
        if a_hidden_states is not None:
            batch, seqlen, dim = a_hidden_states.shape
        if v_hidden_states is not None:
            batch, seqlen, dim = v_hidden_states.shape
        ...
        if a_hidden_states is not None and v_hidden_states is not None:
            return a_out, v_out
        if a_hidden_states is None:
            return a_hidden_states, v_out
        if v_hidden_states is None:
            return a_out, v_hidden_states
